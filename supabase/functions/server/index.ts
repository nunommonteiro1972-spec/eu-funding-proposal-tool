...
// --- KV STORE LOGIC (Inlined) ---
const KV_TABLE_NAME = 'kv_store_3cb71dae';

const KV = {
    async set(key: string, value: any): Promise<void> {
        const supabase = getSupabaseClient();
        const { error } = await supabase.from(KV_TABLE_NAME).upsert({ key, value });
        if (error) throw new Error(`KV set error: ${error.message}`);
    },
    async get(key: string): Promise<any> {
        const supabase = getSupabaseClient();
        const { data, error } = await supabase.from(KV_TABLE_NAME).select('value').eq('key', key).single();
        if (error && error.code !== 'PGRST116') throw new Error(`KV get error: ${error.message}`);
        return data?.value || null;
    },
    async del(key: string): Promise<void> {
        const supabase = getSupabaseClient();
        const { error } = await supabase.from(KV_TABLE_NAME).delete().eq('key', key);
        if (error) throw new Error(`KV delete error: ${error.message}`);
    },
    async getByPrefix(prefix: string): Promise<any[]> {
        const supabase = getSupabaseClient();
        const { data, error } = await supabase.from(KV_TABLE_NAME).select('value').like('key', `${prefix}%`);
        if (error) throw new Error(`KV getByPrefix error: ${error.message}`);
        return data?.map((row: any) => row.value) || [];
    }
};

// --- PROMPT BUILDER LOGIC (Inlined) ---
const PromptBuilder = {
    buildPhase2Prompt(summary: string, constraints: any, userPrompt?: string): string {
    ...
    buildRelevancePrompt(url: string, urlContent: string, constraints: any, ideas: any[], userPrompt?: string): string {
        const basePrompt = userPrompt
    ...
    buildProposalPrompt(idea: any, summary: string, constraints: any, partners: any[] = [], userPrompt?: string, fundingScheme?: any): string {
        const partnerInfo = partners.length > 0
            ? `\n\nCONSORTIUM PARTNERS:\n${partners.map((p: any) => `- ${p.name} (${p.country || 'Country not specified'}): ${p.description || 'No description'}`).join('\n')}`
            : '';
...
const getAI = () => {
    ...
    // Ensure we are using the correct Gemini model/API
        ...
        if (path === '/' || path === '' || path.trim().endsWith('/server') || path.trim().endsWith('/server/')) {
            if (!path.includes('/proposals') && !path.includes('/analyze') && !path.includes('/generate')) {
                ...
                return new Response(JSON.stringify({
                    status: "ok",
                    message: "AI Proposal Generator API (BUNDLED v3.0)",
                    path_detected: path,
                    env_status: {
    ...
    Return ONLY valid JSON.`;
        ...
        if (path.includes('/import-partner-pdf') && req.method === 'POST') {
            ...
            try {
                ...
                // AI Parsing
                const ai = getAI();
                // Reverting to 2.0-flash-exp as requested (Note: changed to 2.5-flash-lite due to availability)
                const model = ai.getGenerativeModel({ model: 'gemini-2.5-flash-lite' });
